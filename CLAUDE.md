...
**Theming**: `src/components/ThemeSwitcher.tsx` lets users swap the `--color-ecom-orange` CSS custom property at runtime (three presets: orange/red/noir), persisted to `localStorage["ecom-theme"]`. This mutates `document.documentElement.style` directly rather than toggling a class — brand colors are Tailwind v4 `@theme` tokens defined in `src/app/globals.css` (`ecom-orange`, `ecom-red`, `ecom-gray`, `ecom-cream`, `ecom-dark`, `ecom-black`), so use those token classes (e.g. `text-ecom-dark`, `bg-ecom-red`) rather than hardcoding hex values.
